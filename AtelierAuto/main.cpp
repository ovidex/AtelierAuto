...
//    std::cout<<m[0]->getAsigurare(1)<<" "<<m[1]->getAsigurare(1)<<" "<<m[2]->getAsigurare(1);
    ...
    a.push_back(new Asistent("Stan","Ana",azi,data{9,12,1990}));

    while (1){
//        std::cout<<isFull(a,"standard")<<"\n";
        for(auto ang:a){
            ang->pass_time();
            if(ang->check_available("standard"));
            std::cout<<"waiting time for "<<ang->getPrenume()<<" "<<ang->waiting_time()<<" queue: ";
            ang->afisare_coada();
            std::cout<<ang->isBusy("standard")<<"\n";
            ang->wait_here("standard");
        }
        ...
        if(command=="atelier"){
            if(a.size()>0) {
                ...
                std::string msg;
                std::cin >> msg;
                if (msg == "da")diesel_Car = true;
                else if (msg == "nu")diesel_Car = false;
                ...
                if(message == "solicita") {
                    if(isFull(a,msg)){
                        ...
                        std::string mesaj;
                        std::cout<<"Doriti un angajat special/primul disponibil(special/primul):\n";
                        std::cin>>mesaj;
                        if(mesaj=="primul"){
                            for(auto angj:a){
                                if(!angj->isBusy(msg))angj->check_available(msg);
                            }
                        }
                            ...
                            for(auto ang:a){
                                if(ang->getID()==id_ang){
                                    if(ang->isBusy(msg)){
                                        std::string msg_ang;
                                        std::cout<<"Angajatul este ocupat!\n"<<"(asteapta/pleaca/primul):\n";
                                        if(msg_ang=="asteapta"){
                                            ang->wait_here(msg);
                                        }
